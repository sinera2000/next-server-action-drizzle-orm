import React from "react";
import { addRoles } from "../api/roles/actions";
import { useRef } from "react";

const page = () => {
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      <h1 className="text-4xl font-bold p-10">Add User Role</h1>
      <form
        action={addRoles}
        className="space-y-4 p-4 max-w-md mx-auto bg-white rounded shadow-md"
      >
        <div className="flex flex-col">
          <label htmlFor="userRole" className="font-medium text-gray-700 mb-1">
            User Role
          </label>
          <input
            className="border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            type="text"
            id="userRole"
            name="role_name"
          />
        </div>

        <div className="flex flex-col">
          <label className="font-medium text-gray-700 mb-1" htmlFor="roleTag">
            Role Tag
          </label>
          <input
            className="border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            type="text"
            id="roleTag"
            name="role_tag"
          />
        </div>

        <button
          className="w-full bg-blue-500 text-white font-medium py-2 rounded hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50"
          type="submit"
        >
          Submit
        </button>
      </form>
    </main>
  );
};

export default page;
